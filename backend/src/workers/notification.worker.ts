...
async function sendEmail(job: Job<NotificationJobData>): Promise<JobResult> {
  logger.info({ jobId: job.id, destination: job.data.destination }, 'Sending email notification');
...
async function sendSms(job: Job<NotificationJobData>): Promise<JobResult> {
  logger.info({ jobId: job.id, destination: job.data.destination }, 'Sending SMS notification');
  await job.updateProgress(50);
...
async function sendPush(job: Job<NotificationJobData>): Promise<JobResult> {
  logger.info({ jobId: job.id, recipientId: job.data.recipientId }, 'Sending push notification');
  await job.updateProgress(50);
...
async function sendSocketNotification(job: Job<NotificationJobData>): Promise<JobResult> {
  logger.info({ jobId: job.id, recipientId: job.data.recipientId, event: job.data.socketEvent }, 'Emitting socket event');
  await job.updateProgress(50);
...
async function processJob(job: Job<NotificationJobData>): Promise<JobResult> {
  switch (job.data.jobType) {
    ...
    default:
      throw new Error(`Unknown notification job type: ${(job.data as NotificationJobData).jobType}`);
  }
}

// ── Worker Instance ─────────────────────────────────────────

export function createNotificationWorker(): Worker<NotificationJobData, JobResult> {
  ...
  worker.on('active', (job) => {
    logger.info({ jobId: job.id, jobType: job.data.jobType }, '[notification-worker] Job started');
  });

  worker.on('completed', (job, result) => {
    logger.info({ jobId: job.id, result }, '[notification-worker] Job completed');
  });
